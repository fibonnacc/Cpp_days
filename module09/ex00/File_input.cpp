...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/02/02 14:58:48 by helfatih          #+#    #+#             */
/*   Updated: 2026/02/02 16:12:19 by helfatih         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "BitcoinExchange.hpp"

std::queue<std::string> split(const std::string &s, const int i)
{
...
void isMoreThanIntMax(std::string &str, const int i)
{
...
{
  std::queue<std::string> tokens = split(str, i);
  ...
  while (!tokens.empty())
  {
    ...
    if (value.empty())
      throw std::runtime_error("Bad input => empty token");

    if (!isFloat(value))
      throw std::runtime_error("Bad input => '" + value + "' is not a number");
    isMoreThanIntMax(value, i);
    tokens.pop();
  }
}
...
void Parsser::ExchangeBitcoin(std::map<std::string, double> &maps)
{
  ...
  for (; it_b1 != it_e1; it_b1++)
  {
    ...
    for (/*Empty*/ ; it_b2 != it_e2; it_b2++)
    {
      if (key1 == it_b2->first)
      {
        value = it_b1->second * it_b2->second;
